        ...
        public static void Main()
        {
            ...
            for (int i = 0; i < GameMaster.GameCount; i++)
            {
                ...
                do
                {
                    ...
                    outputOfJudger = judge.Judger();

                } while (Judge.Draw);

                // intをJankenHandに変換
                JankenHand hand = (JankenHand) Enum.ToObject(typeof(JankenHand), outputOfJudger);

                // 勝者のみをリストから取り出す
                var winnerQuery = players.Where(x => x.Hand == hand);
